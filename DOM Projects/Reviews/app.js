...
const btnRand = document.querySelector(".random-btn");

let id = 0;

const updateUI = () => {
  image.setAttribute("src", reviews[id].img);
  author.textContent = reviews[id].name;
  job.textContent = reviews[id].job;
  info.textContent = reviews[id].text;
};

const randomId = (min, max) => { // min and max included 
...
btnRand.addEventListener("click", ()=>{
    id = randomId(0, reviews.length - 1 )
    updateUI();
})
